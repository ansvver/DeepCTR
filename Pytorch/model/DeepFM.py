...
class DeepFM(torch.nn.Module):
    ...
    def __init__(self,field_size, feature_sizes,
                 embedding_size = 4, is_shallow_dropout = True, dropout_shallow = [0.5,0.5],
                 h_depth = 2, deep_layers = [32, 32],
                 is_deep_dropout = True, dropout_deep=[0.5, 0.5, 0.5],
                 deep_layers_activation = 'relu', n_epochs = 64,
                 batch_size = 256, learning_rate = 0.003,
        ...
        if self.use_fm:
            print("Init fm part")
            #一阶输入embedding处理(实质上embedding维度为1)
    ...
    def forward(self, Xi, Xv):
        ...
        """
            fm part
            fm_first_order_embeddings = nn.ModuleList([nn.Embedding(feature_size,1) 
        ...
        if self.use_fm:
            ...
            fm_first_order = torch.cat(fm_first_order_emb_arr, 1)  #concat操作

            if self.is_shallow_dropout:
                fm_first_order = self.fm_first_order_dropout(fm_first_order)

            # use 2xy = (x+y)^2 - x^2 - y^2
